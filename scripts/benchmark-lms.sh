...
while true; do
  ...
  # CPU% from top (batch mode, 2 iterations for accuracy)
  cpu_pct=$(top -bn2 -d0.5 2>/dev/null | grep "Cpu(s)" | tail -1 \
    | awk '{
        for(i=1;i<=NF;i++) {
          if($i ~ /[0-9]+\.[0-9]+/) {
  ...
  if command -v iostat &>/dev/null; then
    iostat_line=$(iostat -d -m 1 1 2>/dev/null | grep -E "^(sda|vda|nvme|xvda|sdb|vdb|dm-)" | head -1)
