...
_laravel_config() {
  local key=$1
  docker compose exec -T app php -r "require 'vendor/autoload.php'; \$app = require 'bootstrap/app.php'; \$app->make(Illuminate\\Contracts\\Console\\Kernel::class)->bootstrap(); \$value = config('${key}'); if (is_array(\$value)) { echo json_encode(\$value); } elseif (is_bool(\$value)) { echo \$value ? 'true' : 'false'; } elseif (\$value !== null) { echo \$value; }" 2>/dev/null | tr -d '\r\n'
}
