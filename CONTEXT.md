...
**Lightweight Enrolment Model**:
The benchmark uses `course_enrolment_methods` to model active course-level methods such as manual, self, and cohort, plus `course_enrollments` and context role assignments to decide user access. This contrasts with Moodle's `enrol` table, where each plugin instance owns its custom settings, and `user_enrolments`, where each user enrolment points to a specific enrolment instance. The benchmark checks whether a course has an active enrolment method, but it does not reproduce Moodle's per-user enrolment-to-method link, plugin-specific enrolment fields, or multiple active enrolment instances for the same user and course.
...
| **Lightweight Enrolment Model** — course-level `course_enrolment_methods` exist, but user enrolments are not linked to individual method instances | Plugin method instances (`enrol`) + per-instance user records (`user_enrolments`) with plugin fields and timings | Captures active-method checks, but not per-method user enrolment resolution or multi-method-overlap caching | 🟢 Low — active method checks are enough unless enrolment-method behavior becomes benchmarked traffic |
